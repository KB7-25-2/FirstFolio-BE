...
public record LearningContinueResponse(
        ...
        @Schema(description = "소단원 ID. 강좌 이어하기일 때만 제공", example = "101", nullable = true)
        Long subChapterId,
        @JsonInclude(JsonInclude.Include.NON_NULL)
        @Schema(description = "학습 중인 콘텐츠 버전 ID. 강좌 이어하기일 때만 제공", example = "301", nullable = true)
        Long contentVersionId,
        @JsonInclude(JsonInclude.Include.NON_NULL)
        @Schema(description = "진행 중이거나 가장 최근에 실패한 대단원 퀴즈 응시 ID", example = "3001", nullable = true)
        Long attemptId,
        @JsonInclude(JsonInclude.Include.NON_NULL)
        @Schema(description = "JSON 내부 마지막 학습 페이지 ID", example = "page-2", nullable = true)
        String lastPageId,
        @Schema(description = "이어갈 위치의 진행률. 대단원 퀴즈 대상이면 선행 강좌 완료를 나타내는 100", example = "100")
        int progressPercent,
