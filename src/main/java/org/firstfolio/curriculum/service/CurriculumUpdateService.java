...
public class CurriculumUpdateService {
    ...
    public CurriculumUpdateService(
    ...
    @Transactional
    public List<CurriculumDraftItem> update(
            long userId,
        ...
        int removed = userCurriculumMapper.markActiveAsRemoved(userId);
        if (removed != existing.size()) {
            throw new ApiException(ErrorCode.INTERNAL_ERROR);
        }

        int affected = userCurriculumMapper.upsertAll(
                userId,
                items,
                LocalDateTime.now(clock)
        );
        if (affected < items.size()) {
            throw new ApiException(ErrorCode.INTERNAL_ERROR);
        }
        return items;
    }
